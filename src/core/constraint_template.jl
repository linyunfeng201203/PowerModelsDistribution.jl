...
function constraint_mc_power_balance(pm::_PMs.AbstractPowerModel, i::Int; nw::Int=pm.cnw)
    for cnd in _PMs.conductor_ids(pm; nw=nw)
        ...
        #     _PMs.con(pm, nw)[:kcl_p] = Dict{Int,Any}()
        # end
        # if !haskey(_PMs.con(pm, nw, :kcl_p), i)
        #     _PMs.con(pm, nw, :kcl_p)[i] = [undef for i in _PMs.conductor_ids(pm; nw=nw)]
        # end

        # if !haskey(_PMs.con(pm, nw), :kcl_q)
        #     _PMs.con(pm, nw)[:kcl_q] = Dict{Int,Any}()
        # end
        # if !haskey(_PMs.con(pm, nw, :kcl_q), i)
        #     _PMs.con(pm, nw, :kcl_q)[i] = [undef for i in _PMs.conductor_ids(pm; nw=nw)]
        # end
...
function constraint_mc_thermal_limit_from(pm::_PMs.AbstractPowerModel, i::Int; nw::Int=pm.cnw, kwargs...)
    ...
    if haskey(branch, "rate_a")
        for c in _PMs.conductor_ids(pm; nw=nw)
            constraint_mc_thermal_limit_from(pm, nw, c, f_idx, branch["rate_a"])
        end
    end
end
...
function constraint_mc_thermal_limit_to(pm::_PMs.AbstractPowerModel, i::Int; nw::Int=pm.cnw, kwargs...)
    ...
    if haskey(branch, "rate_a")
        for c in _PMs.conductor_ids(pm; nw=nw)
            constraint_mc_thermal_limit_to(pm, nw, c, f_idx, branch["rate_a"])
        end
    end
end
